...
savefileMat = fullfile(opt.pathOutput, ...
    [opt.taskName, 'French-v-Braille_withinModality', '_s', num2str(opt.funcFWHM), '_ratio', num2str(opt.mvpa.ratioToKeep), '.mat']);

savefileCsv = fullfile(opt.pathOutput, ...
    [opt.taskName, 'French-v-Braille_withinModality', '_s', num2str(opt.funcFWHM), '_ratio', num2str(opt.mvpa.ratioToKeep), '.csv']);
...
decodingCondition = {'french_v_braille'};
...
for iSub = 1:numel(opt.subjects)
    ...
    for iImage = 2:length(opt.mvpa.map4D)
